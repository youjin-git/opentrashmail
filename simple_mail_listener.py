...
class SimpleMailHandler:
    ...
    async def handle_DATA(self, server, session, envelope):
        """处理邮件数据"""
        logger.info("🎯 进入 handle_DATA 方法")
        
        try:
            peer_ip = session.peer[0] if session.peer else "unknown"
            logger.info(f"📧 开始处理邮件数据 - 来源IP: {peer_ip}")
            ...
            if not envelope.content:
                ...
                return '250 Message accepted (empty content)'
            
            logger.info(f"   邮件大小: {len(envelope.content)} 字节")
            
            # 显示原始邮件内容的前200字符用于调试
            try:
                raw_preview = envelope.content.decode('utf-8', errors='ignore')[:200]
                logger.info(f"   原始内容预览: {repr(raw_preview)}")
            except Exception as e:
                logger.warning(f"无法预览原始内容: {e}")
